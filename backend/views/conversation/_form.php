...
<script>
    $(document).ready(function() {
        $('#teacher-select').on('change', function() {
            $('input[name="Conversation[tutorId]"]').val('')

            if(this.value.length > 0) {
                $('input[name="Conversation[tutorId]"]').val($(this).find(':selected').data('id'))
            }
        })
    })
</script>
...
<div class="conversation-form">
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', '#conversation-endsat', function() {
                let startsAt = $('#conversation-startsat').val()
                if(startsAt.length === 5) {
                    ...
                    $('#conversation-endsat').val(conversationEnds.getHours() + ':' + conversationEnds.getMinutes())
                }
            })
        })
    </script>
